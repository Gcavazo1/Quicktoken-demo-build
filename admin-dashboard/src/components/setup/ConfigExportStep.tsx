...
const ConfigExportStep: React.FC<ConfigExportStepProps> = ({ 
  ...
  const { allOwners, setIsInSetupMode, isWhitelistLoading, isOwner } = useWhitelist();
  ...
  return (
    <div className="p-6 bg-gray-800 rounded-lg shadow-md">
      ...
      <div className="mb-8 p-4 border-2 border-blue-700 rounded-lg bg-blue-900/30">
         ...
         {/* Actual Export Component - Pass the core config */}
         <div className="mt-6">
           <ConfigExport showTitle={false} configForExport={initialConfig} /> 
         </div>
      </div>
      ...
      <div className="flex justify-between mt-8">
        ...
        <button 
          onClick={onProceed}
          disabled={!isOwner || isConnecting || isNetworkSwitching}
          className={`px-6 py-2 text-white font-medium rounded transition-colors ${
            (!isOwner || isConnecting || isNetworkSwitching) 
              ? 'bg-gray-500 cursor-not-allowed' 
              : 'bg-blue-600 hover:bg-blue-700'
          }`}
          title={!isOwner ? "Connect and verify owner wallet to continue" : "Proceed to final instructions"}
        >
